...
- Strict pop-up guard: a main-world wrapper around `window.open` swallows cross-origin pop-ups and hands the ad script a fake window so it does not fall back to hijacking the current tab, and cross-origin link clicks are blocked. Arms itself automatically on any page where a click-hijack overlay was caught, and can be pinned per site with the popup's "Strict pop-up guard" checkbox
